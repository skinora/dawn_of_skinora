...
class MenuDrawer extends HTMLElement {
  ...
  bindEvents() {
    ...
    this.querySelectorAll(
      'button:not(.localization-selector):not(.country-selector__close-button):not(.country-filter__reset-button):not(.menu-drawer__main-close)',
    ).forEach((button) => button.addEventListener('click', this.onCloseButtonClick.bind(this)));
    this.querySelectorAll('.menu-drawer__main-close').forEach((button) =>
...
(function initKlaviyoConsentPersistence() {
  if (window.__SK_KLAVIYO_CONSENT_PERSIST) return;
  window.__SK_KLAVIYO_CONSENT_PERSIST = true;
  ...
  function getKlaviyoNodes() {
    return Array.from(document.querySelectorAll(klaviyoSelectors.join(', ')));
  }

  function forceShowKlaviyo() {
    const nodes = getKlaviyoNodes();
    if (!nodes.length) return;

    nodes.forEach((node) => {
      const style = node.style;
      if (style.display === 'none') style.display = '';
      if (getComputedStyle(node).display === 'none') style.display = 'block';
      style.setProperty('display', style.display || 'block', 'important');
  ...
  function observeKlaviyo() {
    if (!document.body) return;
    const observer = new MutationObserver((mutations) => {
      for (const mutation of mutations) {
        if (mutation.type === 'attributes') {
          if (mutation.target?.matches?.(klaviyoSelectors.join(', '))) {
            forceShowKlaviyo();
          }
        }
  ...
  function handleConsentClick(event) {
    const button = event.target.closest(
      `${bannerSelectors} button, ${bannerSelectors} [role="button"], ${bannerSelectors} [data-testid]`,
    );
    if (!button) return;

    window.setTimeout(forceShowKlaviyo, 300);
    window.setTimeout(forceShowKlaviyo, 1200);
    startGuard();
    if (window._klOnsite && typeof window._klOnsite.openForm === 'function') {
      window.setTimeout(() => window._klOnsite.openForm('REERkU'), 600);
      window.setTimeout(() => window._klOnsite.openForm('REERkU'), 1600);
    }
  }

  document.addEventListener('click', handleConsentClick, true);
  window.addEventListener('load', forceShowKlaviyo);
  observeKlaviyo();
})();
